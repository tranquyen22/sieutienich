...
import { X, Coins, ArrowUpRight, ArrowDownLeft, Gift, CalendarCheck, CheckCircle2, AlertCircle, Clock, Sparkles } from 'lucide-react';
...
export const CoinWalletModal: React.FC<CoinWalletModalProps> = ({ isOpen, onClose }) => {
  const { userCoins, coinTransactions, dailyCheckIn, hasCheckedInToday } = useShop();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      ...
      >
        {/* Modal Top Banner - Wallet Balance */}
        <div className="bg-gradient-to-br from-amber-500 via-amber-600 to-yellow-600 text-white p-6 relative overflow-hidden shrink-0">
          ...
          <div className="flex items-center gap-2 text-amber-100 text-xs font-extrabold uppercase tracking-wider mb-1">
            ...
            <span>Ví Xu Tiện Ích</span>
          </div>

          <div className="flex items-baseline gap-2 mb-3">
            <span className="text-3xl sm:text-4xl font-black text-white tracking-tight">
              {userCoins.toLocaleString('vi-VN')}
            </span>
            <span className="text-lg font-bold text-yellow-200">Xu</span>
          </div>

          <div className="flex items-center justify-between gap-3 pt-3 border-t border-white/20 text-xs text-amber-100">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full font-extrabold text-xs transition shadow-md cursor-pointer ${
                hasCheckedInToday
            ...
            >
              ...
              <span>{hasCheckedInToday ? 'Đã điểm danh' : 'Điểm danh (+5.000 Xu)'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-3">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 pb-1">
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[11px] font-bold cursor-pointer transition ${
                    historyFilter === 'earn' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  Nhận Xu (+)
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-[11px] font-bold cursor-pointer transition ${
                    historyFilter === 'spend' ? 'bg-rose-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  Tiêu Xu (-)
                </button>
          ...
          {/* TAB 2: NHIỆM VỤ KIẾM XU HÀNG NGÀY */}
          {activeTab === 'tasks' && (
            <div className="space-y-3">
              {/* Task 1: Điểm danh */}
              <div className="p-3.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-500">Mỗi ngày 1 lần bấm điểm danh nhận ngay +5,000 Xu.</p>
                  </div>
                </div>
                ...
                >
                  {hasCheckedInToday ? 'Đã nhận' : '+5.000 Xu'}
                </button>
              </div>

              {/* Task 2: Đăng tin */}
              <div className="p-3.5 bg-white border border-gray-200 rounded-2xl flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center font-bold">
                    <Sparkles className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="font-extrabold text-xs text-gray-900">Đăng tin tiện ích dịch vụ mới</h4>
                    <p className="text-[11px] text-gray-500">Tặng ngay +10,000 Xu khi đăng thành công tin mới.</p>
                  </div>
                </div>

                <span className="px-3 py-1.5 bg-emerald-50 text-emerald-700 font-bold text-xs rounded-xl border border-emerald-200 shrink-0">
                  +10.000 Xu / Tin
                </span>
              </div>
              ...
              <div className="p-3.5 bg-white border border-gray-200 rounded-2xl flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-extrabold text-xs text-gray-900">Thưởng đăng ký tài khoản mới</h4>
                    <p className="text-[11px] text-gray-500">Tặng ngay +50,000 Xu trải nghiệm toàn bộ tiện ích.</p>
                  </div>
                </div>
